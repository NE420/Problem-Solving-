...
int main()
{
	...
	while (n--) {
		...
		if (input == "insert") {
			...
			cin >> num;
			recored[num]++;
			steps.push_back("insert " + to_string(num));
			
		}
		else if (input == "getMin") {
			long long num;
			cin >> num;
			while(!(recored.empty()) && num > recored.begin()->first) {
				for (int i = 0; i < recored.begin()->second; i++)
				{
					steps.push_back("removeMin");
				}
				recored.erase(recored.begin());
			}
			if (!(recored[num])) {
				recored[num]++;
				steps.push_back("insert " + to_string(num));
				steps.push_back("getMin " + to_string(num));
			}
			else {
				steps.push_back("getMin " + to_string(num));
			}
			...
			if (recored.empty()) {
				steps.push_back("insert 1");
				steps.push_back("removeMin");
			}
			else {
				recored.begin()->second--;
				if (recored.begin()->second == 0) {
					recored.erase(recored.begin());
				}
				steps.push_back("removeMin");
			}
